...
class AppSettings(BaseSettings):
    ...
    # ⚠️  ORDEN IMPORTA: las más específicas primero (NASCAR antes de F1, IndyCar antes de F1,
    #     GT regionales antes de GT genérico, etc.)
    CATEGORY_KEYWORDS: Dict[str, List[str]] = {
        "nascar": ["nascar", "cup series", "daytona 500", "brickyard", "southern 500"],
        "indycar": ["indycar", "indianapolis 500", "indy 500"],
        "f1": ["formula 1", "f1", "grand prix"],
        "wrc": ["🚗", "wrc", "rally"],
        "gt_america": ["gt america"],
        "gt_asia": ["gt asia"],
        "gt_australia": ["gt australia"],
        "gt_europe": ["🏁", "gt round", "gt prologue"],
        "wec": ["⏱️", "wec", "le mans"],
    ...
    SESSION_ALIASES: Dict[str, List[str]] = {
        ...
        "q2":           ["q2"],
        # ── Specific — WRC / Rally ────────────────────────────────────────
        "rally":        ["rally", "ss", "special stage", "shakedown"],
    }
...
async def get_calendar_ics(
    cats:           Optional[List[str]] = Query(None, description="Categorías: f1, gt_europe, gt_america, gt_asia, gt_australia, nascar, motogp, wrc, wec, indycar"),
    ...
    gt_europe_sessions:   Optional[List[str]] = Query(None, description="Sesiones GT Europe: fp1, fp2, qualifying, race"),
    gt_america_sessions:  Optional[List[str]] = Query(None, description="Sesiones GT America: qualifying, race"),
    gt_asia_sessions:     Optional[List[str]] = Query(None, description="Sesiones GT Asia: race"),
    gt_australia_sessions:Optional[List[str]] = Query(None, description="Sesiones GT Australia: race"),
    ...
    wrc_sessions:         Optional[List[str]] = Query(None, description="Sesiones WRC: rally"),
    wec_sessions:         Optional[List[str]] = Query(None, description="Sesiones WEC: qualifying, race"),
    indycar_sessions:     Optional[List[str]] = Query(None, description="Sesiones IndyCar: race"),
    ...
    try:
        ...
        _raw = {
            "f1":           f1_sessions,
            "gt_europe":    gt_europe_sessions,
            "gt_america":   gt_america_sessions,
            "gt_asia":      gt_asia_sessions,
            "gt_australia":  gt_australia_sessions,
            "nascar":       nascar_sessions,
            "motogp":       motogp_sessions,
            "wrc":          wrc_sessions,
            "wec":          wec_sessions,
            "indycar":      indycar_sessions,
        }
